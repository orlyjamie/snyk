...
import config = require('../../../src/lib/config');
...
export const GenericTests: AcceptanceTests = {
  ...
  tests: {
    ...
    'print-deps w/ muted cli logs for big tree': (params, utils) => async (t) => {
      // params.cli.config('set','PRUNE_DEPS_THRESHOLD=0'); // make sure we get a "big" tree
      // params.cli.config('set','api=123456789'); // make sure we get a "big" tree
      utils.chdirWorkspaces('print-deps');
      try {
        ...
        console.log(err);
        t.fail('Something went wrong with CLI call');
      } finally {
        params.cli.config('set',`PRUNE_DEPS_THRESHOLD=${config.PRUNE_DEPS_THRESHOLD}`);
      }
      t.end();
    },
    //
